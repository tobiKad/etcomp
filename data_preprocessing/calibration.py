class Calibration():
    def __init__(self):
        ...
        for i in range(1,counter):
            df = pd.read_csv('./data/lb_data/trial_data/p' + str(i) + '_trials.csv',encoding='utf-8')
            dftemp.append(df)
        all_lb = pd.concat(dftemp, axis=0, ignore_index=True)
        ...
        # plt.ylabel("Measured samples")
        # plt.xlabel("time in miliseconds")
        # plt.title('Histogram which shows reaction time of pressing space button after target was not visible for participant \n with FILTER LESS THEN 500ms over all participants')
        # # show plot
        ...
        all_lb.reset_index(inplace=True)
        all_lb.to_csv('./data/lb_data/lb_trial_pp/p' + str(i) +'_trial_pp.csv', index=False)